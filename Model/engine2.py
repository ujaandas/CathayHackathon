...
content_df2 = data[["ID", "Fruit", "FlightID", "Code", "Arrival Date", "Via"]]

# Join non-null values of each row
content_df2["Content"] = content_df2.apply(
    lambda row: " ".join(row.dropna().astype(str)), axis=1
)

# # Use TF-IDF vectorizer to convert content into a matrix of TF-IDF features
# tfidf_vectorizer = TfidfVectorizer()
# content_matrix = tfidf_vectorizer.fit_transform(content_df2["Content"])
...
#     index = content_df2[content_df2["FruidID"] == product_id].index[0]
#     similarity_scores = content_similarity[index]
#     similar_indices = similarity_scores.argsort()[::-1][1 : top_n + 1]
#     recommendations = content_df2.loc[similar_indices, "ID"].values
...
#     return content_df2[content_df2["ID"] == product_id]["Fruit"].iloc[0]
