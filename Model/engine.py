...
data = Dataset.load_from_df(data[["ID", "FruitID", "Score"]], reader)


# Use SVD to train the model
def get_content_based_recommendations(product_id, top_n):
    index = content_df[content_df["Fruit"] == product_id].index[0]
    similarity_scores = content_similarity[index]
    similar_indices = similarity_scores.argsort()[::-1][1 : top_n + 1]
    recommendations = content_df.loc[similar_indices, "Fruit"].values
...
def convert_product_id_to_name(product_id):
    return content_df[content_df["FruitID"] == product_id]["Fruit"].iloc[0]


# Test the function
# print(get_collaborative_filtering_recommendations("510892B00001637F", 5))
...
def recommend(params: dict):
    ...
    # Get recommendations
    recommendations = get_hybrid_recommendations(user_id, fruit_name, top_n)
    output = []

    print(f"Hybrid Recommendations for User {user_id} based on Product {fruit_name}:")
...
if __name__ == "__main__":
    recommend({"user_id": "510892B00001637F", "fruit_name": "Pineapple", "top_n": 5})
