  ...
  return (
    <>
      ...
      <div
        ...
        className={`mx-5 cursor-pointer align-middle items-center text-center text-black  shadow-xl p-5 rounded-lg transition-all ${
          selected == title ? "bg-sky-100 " : "bg-white hover:border-blue-500"
        }`}
      >
        <h1 className="font-extrabold ">{title}</h1>
        <ul>
          {description.map((desc) => (
            <li key={desc} className="custom-li">
              {desc}
            </li>
          ))}
        </ul>
